...
AgentBase {
    domain: "www.coinexplorer.net"

    property int reqPing: -1
    property string subPing: "/api/v1/LTC/block/latest"
    property string subUtxo: "/api/v1/LTC/address/unspent?address=:addr:"
    property string subTx: "/api/v1/LTC/transaction?txid=:txid:"
    ...
    function sendTransaction(txid,rawtx) {
        // not implement, use   https://litecoinblockexplorer.net/sendtx
        var rawPost = "hex=" + rawtx
        var jsonObj = {"rawpost": rawPost,
            "Content-Type": "application/x-www-form-urlencoded",
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8"}
        JsonRpc.rpcCall("", jsonObj, "litecoinblockexplorer.net-btx:"+txid,
                        "litecoinblockexplorer.net", port, tls, "/sendtx")
        reqCount++
    }
...
//            }
            ...
            for (var i = 0; i < utxoset.length; i++) {
                ...
                jdata[i]["satoshis"] = Config.coinsAmountValue(utxoset[i]["value"], "LTC")
                jdata[i]["confirmations"] = 24 // not supply
            }
    ...
    function handleTransactions(reply) {
        ...
        try {
            ...
            if (   typeof(reply["result"]) != "undefined"
                && typeof(reply["result"]["includedInBlock"]) != "undefined") {
                var inblock = parseInt(reply["result"]["includedInBlock"])
                if (blockHeight >= inblock) {
                    confirms = blockHeight - inblock + 1
    ...
    Connections {
        ...
        onRpcReply: {
            ...
            if (reply["user_data"].startsWith("litecoinblockexplorer.net-btx:")) {
