...
AgentBase {
    domain: "api.smartbit.com.au"

    property int reqPing: -1
    property string subPing: "/v1/blockchain/blocks?limit=1"
    property string subUtxo: "/v1/blockchain/address/:addr:/unspent?limit=1000"
    property string subTx: "/v1/blockchain/tx/:txid:"
    property string subSendTx: "/v1/blockchain/pushtx"
    ...
    function handlePing(reply) {
        ...
        try {
            ...
            var bh = reply["blocks"]["0"]["height"]
            if (blockHeight < bh) {
                blockHeight = bh
                var size = reply["blocks"]["0"]["size"]
                var fee  = reply["blocks"]["0"]["fees_int"]
                feeperbyte = Math.round((fee / size) * 100) / 100
                var tm  = reply["blocks"]["0"]["time"]
...
//            }
            ...
            for (var i = 0; i < utxoset.length; i++) {
                ...
                jdata[i]["vout"] = utxoset[i]["n"]
                jdata[i]["satoshis"] = utxoset[i]["value_int"]
    ...
    function handleTransactions(reply) {
        ...
        try {
            var txreq = reply["user_data"]
            var txid = txreq.replace(/[^:]*:([^:]*)/,"$1")
            if (typeof(reply["transaction"]) != "undefined") {
                confirms = 0
                if (typeof(reply["transaction"]["confirmations"]) != "undefined") {
                    confirms = reply["transaction"]["confirmations"]
                }
