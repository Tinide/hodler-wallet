...
Rectangle {
    ...
    function refreshList() {
        for (var i = 0; i < agent.servList.length; i++) {
            listModel.set(i, {id: i, ct: agent.servList[i].coinType, st: agent.servList[i].status})
        }
    }
    ...
    QList {
        ...
        delegate: ApiServiceListItem {
            ...
            status: st
            onClicked: {
                pageServiceList.coinType = ct
                pageServiceList.serviceID = id
                pageServiceList.show()
            }
        }
    }
