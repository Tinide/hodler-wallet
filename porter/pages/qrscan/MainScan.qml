...
Rectangle {
    ...
    Connections {
        ...
        onQrScanResult: {
            ...
            try {
                ...
                if (json["m"] !== "TXS") {throw Lang.msgUnknownQRData}
                ...
                if (curPage === maxPage) {
                    ...
                    //console.info(qrdata)
                    pageTransactionResult.loadJsonData(qrdata)
                    pageTransactionResult.show()
                ...
                if (pageAddressResult.parseAddress(qrd)) {
                    loaderScanner.active = false
                    loaderScanner.source = ""
                }
            }
    ...
    TransactionResult {
        id: pageTransactionResult
        anchors.fill: parent
        onBackClicked: {
            pageTransactionResult.hide()
            _pageScan.backClicked()
        }
    }

    AddressResult {
        id: pageAddressResult
        anchors.fill: parent
        onBackClicked: {
            pageAddressResult.hide()
            _pageScan.backClicked()
        }
        onAddClicked: {
            pageAddressResult.hide()
            _pageScan.backClicked()
            Config.scanAddAddress(address, label, ct)
        }
    }
